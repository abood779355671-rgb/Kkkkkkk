...
log.info("تهيئة MongoDB")
mongo_client = MongoClient(MONGO_URL)
db = mongo_client.wbb

async def load_sudoers():
    global SUDOERS
    log.info("تحميل المستخدمين المطورين")
...
if not SESSION_STRING:
    app2 = Client(name="sessions/userbot", api_id=API_ID, api_hash=API_HASH, phone_number=PHONE_NUMBER)
else:
    app2 = Client(name="sessions/userbot", api_id=API_ID, api_hash=API_HASH, session_string=SESSION_STRING)

aiohttpsession = ClientSession()
arq = ARQ(ARQ_API_URL, ARQ_API_KEY, aiohttpsession)
app = Client("sessions/wbb", bot_token=BOT_TOKEN, api_id=API_ID, api_hash=API_HASH)

log.info("تشغيل البوت الرئيسي")
app.start()
log.info("تشغيل بوت المستخدم (userbot)")
app2.start()

log.info("جلب معلومات الحسابات")
x = app.get_me()
y = app2.get_me()
